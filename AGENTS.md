...
- When the user says to work on one specific page, keep changes isolated to that page. If editing shared components such as `components/product-page.tsx`, first check what other pages use them, then gate page-specific behavior with an explicit condition such as `isModularHomesPage` or create a page-specific component. Before saying the work is done, review `git diff` for unintended shared behavior changes, not only lint/build results.
- The site header, desktop navigation, dropdown menus, mobile menu, and footer always stay dark, even on light-themed pages such as project detail pages.
